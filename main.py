...
# ✅ Webhook handlers respond FAST (prevents Telegram "Read timeout")
# ✅ Startup never blocks (prevents site stuck loading)
# ------------------------------------------------------------
...
STATE: Dict[str, Any] = {
    ...
    "customs_map": {},           # customs_root_id -> destination
    "bindings_by_client": {},    # client_key -> binding details
...
async def ensure_telegram_webhook() -> None:
    ...
    log("Setting Telegram webhook:", hook_url)
...
def drive_find_client_folder_exact_under_root(client_name: str) -> Optional[Dict[str, str]]:
    ...
    # NOTE: This may be slow if you have MANY client folders.
    # We'll keep your logic but now it runs in background so it won't time out Telegram.
...
def start_watch() -> Dict[str, Any]:
    ...
    log("Drive watch started. expiration_ms=", STATE["expiration_ms"])
    return res
...
async def watch_renewer_loop():
    ...
    while True:
        try:
            ...
            if not exp_ms:
                log("No expiration set, renewing watch…")
                renew_watch_now()
            ...
            if seconds_left <= RENEW_AHEAD_SECONDS:
                log("Renewing watch (seconds_left=", seconds_left, ")")
                renew_watch_now()
                await asyncio.sleep(MIN_CHECK_SECONDS)
                continue
            ...
            log("watch_renewer_loop error:", repr(e))
            await asyncio.sleep(MIN_CHECK_SECONDS)


# ---------------- STARTUP / SHUTDOWN (NON-BLOCKING) ----------------
...
async def on_startup():
    ...
    async def bootstrap():
        # Do external calls in background so the web server becomes responsive immediately
        try:
            await ensure_telegram_webhook()
        except Exception as e:
            log("ensure_telegram_webhook failed:", repr(e))

        try:
            renew_watch_now()
        except Exception as e:
            log("renew_watch_now failed:", repr(e))
...
# ---------------- TELEGRAM UPDATE HANDLER (BACKGROUND) ----------------
async def handle_telegram_update(update: dict):
    try:
        ...
        if text.lower().startswith("/register"):
            ...
            # Optional: immediate feedback so user sees it’s working
            await tg_send(chat_id, f"🔎 Looking for: {client_input} …", thread_id=thread_id)

            try:
                client_folder_id, client_folder_name, onlyfans_id, customs_id, customs_name = find_client_onlyfans_customs(client_input)
            except Exception as e:
                log("register lookup failed:", client_input, repr(e))
                await tg_send(
                    chat_id,
        ...
        log("handle_telegram_update crashed:", repr(e))
...
async def telegram_webhook(secret: str, request: Request):
    ...
    # IMPORTANT: respond immediately to Telegram to avoid timeouts
